...
loop:
	for scanner.Scan() {
		...
		bts, err = transformMessageField(bts, log)
		if err != nil {
			slog.Error("failed to transform MESSAGE field", "error", err)
			continue
		}
...
func transformMessageField(bts []byte, log autoGeneratedJournal) ([]byte, error) {
	var rawMessage json.RawMessage
	err := json.Unmarshal([]byte(log.Message), &rawMessage)
	if err == nil {
		// if the message is valid JSON, set it directly (unescape the json-escaped-string)
		bts, err = sjson.SetBytes(bts, "MESSAGE", rawMessage)
		if err != nil {
			return nil, fmt.Errorf("failed to set MESSAGE: %w", err)
		}
		bts, err = sjson.SetBytes(bts, "_MESSAGE_IS_JSON", true)
		if err != nil {
			return nil, fmt.Errorf("failed to set _MESSAGE_IS_JSON: %w", err)
		}
	} else {
		bts, err = sjson.SetBytes(bts, "MESSAGE.text", log.Message)
		if err != nil {
			return nil, fmt.Errorf("failed to set MESSAGE.text: %w", err)
		}
	}
	return bts, nil
}
